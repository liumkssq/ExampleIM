...
var (
	...
	cacheEasyChatFriendRequestsIdPrefix = "cache:easyChat:friendRequests:id:"
)
...
func (m *defaultFriendRequestsModel) Update(ctx context.Context, session sqlx.Session, data *FriendRequests) error {
	friendRequestsIdKey := fmt.Sprintf("%s%v", cacheEasyChatFriendRequestsIdPrefix, data.Id)
...
func (m *defaultFriendRequestsModel) Delete(ctx context.Context, id int64) error {
	friendRequestsIdKey := fmt.Sprintf("%s%v", cacheEasyChatFriendRequestsIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
...
func (m *defaultFriendRequestsModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheEasyChatFriendRequestsIdPrefix, primary)
}
