...
var (
	...
	cacheEasyChatGroupsIdPrefix = "cache:easyChat:groups:id:"
)
...
func (m *defaultGroupsModel) Delete(ctx context.Context, id string) error {
	groupsIdKey := fmt.Sprintf("%s%v", cacheEasyChatGroupsIdPrefix, id)
...
func (m *defaultGroupsModel) Insert(ctx context.Context, session sqlx.Session, data *Groups) (sql.Result, error) {
	groupsIdKey := fmt.Sprintf("%s%v", cacheEasyChatGroupsIdPrefix, data.Id)
...
func (m *defaultGroupsModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheEasyChatGroupsIdPrefix, primary)
}
